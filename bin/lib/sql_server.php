    ...
    public function MsSQL(){
        ...
        if(!empty($_SESSION["conectDB"])){
            ...
            if(isset($_SESSION["db"]["host"]) && isset($_SESSION["db"]["user"]) && isset($_SESSION["db"]["password"]) && isset($_SESSION["db"]["db_name"])){
                ...
                $_SESSION["conectDB"] = @mssql_connect($_SESSION["db"]["host"], $_SESSION["db"]["user"], $_SESSION["db"]["password"]);
                //bin_debug($_SESSION);
                if($_SESSION["conectDB"]){

                    if(mssql_select_db($_SESSION["db"]["db_name"], $_SESSION["conectDB"])){
                        $this->conexion = $_SESSION["conectDB"];
                    ...
                    //$this->MsSQL();
                    die("Error al conectar con el servidor, por favor recarge la paginaaaaaaaaaaaaaaa1");
                }
            }
            else{
                die("Error al conectar con el servidor, por favor recarge la paginaaaaaaaaaaaa2");
            }
        }
    }
